...
def profile_model_memory(
    ...
    try:
        ...
        print(
            f"Error loading {model_name}: {e}. Initializing a mock configuration instead."
        )
        # Fallback to random weights if offline or Hugging Face fails
        config = T5Config(
            d_model=d_model, num_layers=6 if "small" in model_name else 12
        )
    ...
    labels = torch.randint(
        100, 3000, (batch_size, target_len), dtype=torch.long, device=device
    )
    ...
    if device.type == "cuda":
        ...
        print(
            "Note: To get accurate VRAM profiles, run this script inside a Kaggle GPU notebook."
        )
...
def main() -> None:
    ...
    profile_model_memory(
        model_name="t5-small", batch_size=8, seq_len=150, target_len=30, d_model=512
    )
