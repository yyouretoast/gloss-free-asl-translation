...
def export_encoder_to_onnx(
        ...
        if os.path.isdir(model_path):
            ...
            if os.path.exists(safetensors_path):
                ...
                raise FileNotFoundError(
                    f"No checkpoint weights (model.safetensors or pytorch_model.bin) found in directory {model_path}"
                )
    ...
    print(
        f"Initializing model for ONNX export (input_dim={input_dim}, input_i3d_dim={input_i3d_dim}, t5_model_name={t5_model_name})..."
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Export ASL Conformer Encoder to ONNX."
    )
    parser.add_argument(
        "--model-path",
        "--model_path",
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="results/conformer_encoder.onnx",
    ...
    parser.add_argument(
        ...
        type=int,
        default=512,
        help="Conformer embedding dimension.",
    )
    parser.add_argument(
        "--num-layers",
        "--num_layers",
    ...
    parser.add_argument(
        ...
        type=int,
        default=31,
        help="Conformer conv kernel size.",
    )
    parser.add_argument(
        "--t5-model",
        "--t5_model",
        ...
        type=str,
        default="t5-small",
        help="T5 model name used during training.",
    )
