...
def _compute_missing_rates(
    left_hand: np.ndarray, right_hand: np.ndarray, face: np.ndarray, num_frames: int
    ...
    return (
        left_hand_missing_pct,
        right_hand_missing_pct,
        face_missing_pct,
        left_hand_missing,
        right_hand_missing,
        face_missing,
...
def _compute_wrist_noise(
    pose: np.ndarray,
    left_hand_missing: np.ndarray,
    right_hand_missing: np.ndarray,
    left_wrist_idx: int,
    right_wrist_idx: int,
) -> float:
    ...
    left_valid = (np.linalg.norm(left_wrists, axis=1) > 1e-5) & (
        ~np.isnan(left_wrists).any(axis=1)
    )
    right_valid = (np.linalg.norm(right_wrists, axis=1) > 1e-5) & (
        ~np.isnan(right_wrists).any(axis=1)
    )
        ...
        if file_path.endswith(".npy"):
            ...
            with np.load(file_path) as data:
                ...
                if not required_keys.issubset(data.files):
                    print(
                        f"Warning: {file_path} is missing keys. Found: {list(data.files)}"
                    )
                    return None
...
def validate_dataset(directory_path: str, limit: int = 50) -> Optional[Dict[str, Any]]:
    ...
    if limit and len(files) > limit:
        print(
            f"Dataset contains {len(files)} files/folders. Limiting analysis to first {limit} items for speed."
        )
        files = files[:limit]
    ...
    print(
        f"  Left Hand Missing:       {lh_missing_arr.mean():.1f}% avg (max: {lh_missing_arr.max():.1f}%)"
    )
    print(
        f"  Right Hand Missing:      {rh_missing_arr.mean():.1f}% avg (max: {rh_missing_arr.max():.1f}%)"
    )
    print(
        f"  Face Mesh Missing:       {face_missing_arr.mean():.1f}% avg (max: {face_missing_arr.max():.1f}%)"
    )
    print("-" * 60)
...
def generate_mock_dataset(directory_path: str, num_files: int = 5) -> None:
    ...
    for i in range(num_files):
        ...
        np.savez_compressed(
            filepath, pose=pose, left_hand=left_hand, right_hand=right_hand, face=face
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--data_dir", type=str, default="data/landmarks", help="Path to landmark files."
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=50,
        help="Maximum number of files/folders to profile.",
    )
    ...
    if not os.path.exists(dir_path) or not files:
        ...
        if dir_path.startswith("/kaggle/input") or not os.access(
            os.path.dirname(dir_path) or ".", os.W_OK
        ):
            import sys

            print(
                f"Error: Dataset directory {dir_path} does not exist and is in a read-only partition."
            )
            sys.exit(1)
