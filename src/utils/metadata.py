...
def load_metadata(metadata_file: str | None) -> Tuple[Dict[str, str], Dict[str, str]]:
    ...
    if metadata_file:
        ...
        file_candidates = [
            c
            for c in df.columns
            if any(x in c.lower() for x in ["id", "file", "video", "key", "name"])
        ]
        ...
        text_col = [
            ...
            if any(
                x in c.lower()
                for x in ["text", "trans", "gloss", "sentence", "caption"]
            )
        ...
        signer_col = [
            ...
            if any(
                x in c.lower()
                for x in ["signer", "channel", "uploader", "author", "subject"]
            )
        ]
        ...
        if file_col and text_col:
            ...
            if signer_col:
                ...
                video_to_signer = dict(
                    zip(df[f_col].astype(str), df[s_col].astype(str))
                )
        else:
            raise ValueError(
                f"Could not find matching columns. Columns: {df.columns.tolist()}"
            )
    else:
