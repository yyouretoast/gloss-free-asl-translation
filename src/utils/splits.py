...
def split_by_signer(
    filepaths: List[str], video_to_signer: Dict[str, str]
    ...
    if len(sorted_signers) > 0:
        ...
        sorted_signers_by_size = sorted(
            sorted_signers, key=lambda s: len(signer_groups[s]), reverse=True
        )
        ...
        for signer in sorted_signers_by_size:
            ...
            if (
                len(train_files) == 0
                or (len(train_files) + len(files)) / total_known_count <= 0.85
            ):
        ...
        print(
            f"Signer splits: {len(train_files)} train (includes {len(unknown_files)} unknown-signer clips), {len(val_files)} validation files."
        )
    else:
        # Fallback to random split if signer info is unavailable.
        print("\n" + "=" * 80)
        print(
            "WARNING: No signer, channel, or uploader metadata found in filenames or CSV columns."
        )
        print(
            "Falling back to standard random split. Note: validation metrics may suffer from signer data leakage."
        )
        print("=" * 80 + "\n")
        ...
        print(
            f"Fallback splits: {len(train_files)} train, {len(val_files)} validation files."
        )
