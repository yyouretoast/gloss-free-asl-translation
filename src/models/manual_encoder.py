...
class ConformerConvModule(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, key_padding_mask: Optional[torch.BoolTensor] = None
    ) -> torch.Tensor:
        """
        Args:
            x: Input tensor of shape (batch, seq_len, d_model).
    ...
    def forward(
        ...
        if key_padding_mask is not None:
            ...
            col_indices = torch.arange(
                key_padding_mask.size(1), device=key_padding_mask.device
            )
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, max_len: int = 10000) -> None:
        ...
        self.register_buffer(
            "pe", self._get_pe(max_len, torch.device("cpu"), torch.float32)
        )
...
class ConformerEncoder(nn.Module):
    ...
    def forward(
        ...
        if x.size(1) == 0:
            raise ValueError(
                f"Input feature sequence has length 0. Batch size: {x.size(0)}"
            )
        ...
        if attention_mask is not None:
            ...
            downsampled_mask = F.max_pool1d(
                attention_mask.unsqueeze(1).float(), kernel_size=3, stride=2, padding=1
            ).squeeze(1)
